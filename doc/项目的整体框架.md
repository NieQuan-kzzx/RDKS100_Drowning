...
首先，按照已有的例子，创建.cpp和.h文件，查找是否之前有现成的例子，按照例子上的内容进行修改，能调用地平线提供的API能更简单，这里不仔细讲解如何创建一个新的类，具体的可以查看代码自行理解。

当有了这两个类之后，首先前往DetextionWorker.cpp的switchModel中添加初始化，如何初始化，看到前面已经初始化的捏内容自然能够理解，创建好之后，再去MainWindows.cpp里的on_btnConfirm_clicked里面添加对应ComboBox的标签，具体如何添加也是一看就懂。
