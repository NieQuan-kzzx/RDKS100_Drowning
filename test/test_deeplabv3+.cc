...
int main(int argc, char **argv) {
    ...
    // 指定模型文件路径
    const char* model_file_name = "/home/sunrise/Desktop/RDKS100_Drowning/models/deeplabv3plus_efficientnetm2_1024x2048_nv12.hbm";
    const char* image_file_name = "/home/sunrise/Desktop/RDKS100_Drowning/tem/test.png";
    /*
    ...
    */
    ...
    for (int i = 0; i < output_count; i++) {
        ...
        // 保存纯色分割图
        save_segmentation_result(seg_mask, w, h, "deeplabv3+_mask.png");

        // 保存半透明叠加图
        if (!original_img.empty()) {
            cv::Mat resized_src;
            cv::resize(original_img, resized_src, cv::Size(w, h)); // 确保原图尺寸与输出一致
            blend_segmentation(resized_src, seg_mask, "deeplabv3+_blended.png");
        }
    }
    // 释放资源
